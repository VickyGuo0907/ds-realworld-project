...
def sample_dataset():
    ...
    data = {
        ...
        "department": np.random.choice(
            ["Engineering", "Sales", "HR", "Finance"], n_samples
        ),
...
class TestEndToEndPipeline:
    ...
    def test_complete_ml_pipeline(self, sample_dataset, mlflow_experiment):
        ...
        assert (
            df_processed[numeric_cols].notna().all().all()
        ), "Numeric cols should not be null"
        ...
        assert all(
            "accuracy" in r["metrics"] for r in results
        ), "Accuracy should be in metrics"
    ...
    def test_data_loading_and_validation(self, sample_dataset):
        ...
        assert all(
            v == 0 for v in info["null_counts"].values()
    ...
    def test_preprocessing_chain(self, sample_dataset):
        ...
        assert df_encoded["department"].dtype in [
            np.int64,
            np.int32,
        ], "Categorical col should be numeric"

        preprocessor.df = df_encoded
        df_normalized = preprocessor.normalize_features()
        assert (
            abs(df_normalized[numeric_cols].mean().mean()) < 0.1
    ...
    def test_feature_engineering_comprehensive(self, sample_dataset):
        ...
        assert (
            "income_x_experience" in df_inter.columns
        ), "income_x_experience should exist"
    ...
    def test_data_splitting_stratification(self, sample_dataset):
        ...
        assert (
            abs(train_ratio - overall_ratio) < 0.1
        ), "Train ratio should match overall"
...
class TestPipelineErrorHandling:
    ...
    def test_splitter_invalid_proportions(self, sample_dataset):
        ...
        with pytest.raises(ValueError):
            splitter.train_val_test_split(train_size=0.5, val_size=0.3, test_size=0.3)
...
class TestPipelineIntegration:
    ...
    def test_feature_engineer_maintains_row_count(self, sample_dataset):
        ...
        assert (
            len(df_poly) == original_rows
        ), "Row count should be preserved after polynomial features"
        assert (
            len(df_inter) == original_rows
        ), "Row count should be preserved after interaction features"
